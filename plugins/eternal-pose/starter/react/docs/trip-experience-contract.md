...
The pure trip model, validation, scheduling, progress, route, timeline, map-presentation, and sheet-geometry invariants are provided by the exact pinned `@laugh-tale-island/core` version, and the selection, progress, route, location, candidate-decision, and sheet behavior hooks by the exact pinned `@laugh-tale-island/react` version; upgrade the packages rather than re-implementing them locally.
